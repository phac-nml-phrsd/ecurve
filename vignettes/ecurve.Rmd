...
The package provides functionality for performing the MLE via the `esc_mle()` function, for estimating the concentration of an unknown sample from qPCR data and a fitted model via the `conc_mle()` function, and for using Bayesian methods to generate a credible interval to quantify the uncertainty in such a concentration estimate via the `conc_interval()` function. The `multi_interval()` function is also provided for performing concentration estimations for multiple samples at once using the same standard curve.

Note that the functions `esc_mle()`, `conc_mle()`, `conc_interval()`, `multi_interval()`, and `plot_esc_model()` muat all internally compute features of the theoretical distribution of $C_q$ values at a given concentration by summing over possible $N_0$ values, and since the number of possible $N_0$ values in infinite, this sum must be approximated by considering a finite subset of the possible values. Testing indicates that the default approximation used works well most of the time, but may break down at extremely small values of $\sigma$. Although such scenarios are unlikely to occur in practice, a more accurate approximate using more $N_0$ values, which perfoms well even in these extreme cases, can be specificed by setting the optional parameter `approximate = FALSE` in any of these functions. However, it should be noted that using this alterante approximation may cause significant slowdowns when working with high concentration values, and provides minimal benefits outside of such extreme cases, which is why it is not used by default.
...
Finally, we illustrate the use of the `multi_interval()` function to fit concentration credible intervals for all samples in the `esc-test-data.csv` dataset simultaneously. The `test.data` dataframe is already in the format required by this function, with a `sample` column listing the sample indices and and `cqs` column listing the corresponding $C_q$ values. For each unique sample index, this function computes the MLE concentration extimate and lower and upper bounds of a Bayesian credible interval based on the given $C_q$ values, and packages the results into a dataframe. As with the `conc_interval()` function, there is also an optinal `level` parameter to control the level of the credible intervals, with the default being 0.95

```{r, warning = FALSE}
multi_interval(test.data, model = model)
```

Note that the intervals for samples 3, 7, and 8 are the same as those produced by the `conc_interval()` function above; however, there is no mechanism for poltting the intervals generated by the `multi_interval` function.
