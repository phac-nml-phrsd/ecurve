...
In addition to fitting the ESC model, the `ecurve` package provides functionality for performing the MLE via the `esc_mle()` function, for estimating the concentration of an unknown sample from qPCR data and a fitted model via the `conc_mle()` function. 
...
To fit the ESC model to the data, we use the `esc_mle()` function. This function takes a dataframe of the known concentrations and $C_q$ values used to calibrate the model.
